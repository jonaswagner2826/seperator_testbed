classdef nlfeedback < nlcomposite
    ...
    methods
        function sys = nlfeedback(sys1, sys2, t, u)
            ...
            %actual input
%             sys.u = u;
            sys.control_signal = sys.u; %(assuming zero-state feedback loop)
        end
    ...
    methods
        function sys = update(sys,r,t,x)
            ...
            % Input Defaults
            if nargin < 2 | r == 0
                r = zeros(sys.p,1); % stabilization (r = 0)
            end
            if nargin < 3 || t == -1
            ...
            sys1 = sys.sys1.update(sys.control_signal,t,x1);
            x1 = sys1.x;
            y1 = sys.sys1.h(x1,sys.control_signal);
            ...
            sys.sys2 = sys2;
            sys.x = [x1;x2];
            sys.u = r; %closed loop system input, not control signal
            sys.control_signal = u_local;
            sys.y = y1;
            sys.t = sys1.t;
        end
